...
Triajo is a task triage system. Tasks come in, get tagged by handler type, and route to whoever can do them. The person at the center sets the rules and makes decisions. Everything else is delegated.
...
2. **Agent-native**: AI agents are first-class handlers. They can create tasks, claim tasks, update status, and broadcast completions. The MCP server makes triajo accessible from any Claude session.
